@extends('layouts.app')
@section('title', 'Editar Zona / Aula')
@section('content')
    <div class="page-header"><h1 class="page-title">Editar Zona / Aula: {{ $aula->nombre }}</h1></div>
    ...
    <div class="card" style="max-width: 700px;">
        <form method="POST" action="{{ route('aulas.update', $aula) }}">
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <select name="tipo" id="tipo-select" onchange="toggleCapacidad()" required
                        ...
                        <option value="aula" {{ old('tipo', $aula->tipo ?? 'aula') === 'aula' ? 'selected' : '' }}>Aula</option>
                        <option value="zona" {{ old('tipo', $aula->tipo ?? 'aula') === 'zona' ? 'selected' : '' }}>Zona</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>Identificación (Código / Siglas)</label>
                    <input type="text" name="identificacion" value="{{ old('identificacion', $aula->identificacion) }}" placeholder="Ej: A-101, Z-01">
                </div>
            </div>

            <div class="form-group">
                <label>Nombre del espacio *</label>
                <input type="text" name="nombre" value="{{ old('nombre', $aula->nombre) }}" required placeholder="Ej: Aula de Informática, Biblioteca, Patio...">
            </div>

            <div class="grid-2">
                <div class="form-group" id="capacidad-group">
                    <label>Capacidad (opcional)</label>
                    <input type="number" name="capacidad" id="capacidad-input" value="{{ old('capacidad', $aula->capacidad) }}" min="1" max="500" placeholder="Ej: 30">
                </div>
                <div class="form-group">
                    <label>Ubicación</label>
                    <input type="text" name="ubicacion" value="{{ old('ubicacion', $aula->ubicacion) }}" placeholder="Ej: Planta 1, Edificio Principal">
                </div>
            </div>
                    ...
                    @foreach($equipamientoOptions as $equip)
                        <div class="form-check">
                            <input type="checkbox" name="equipamiento[]" value="{{ $equip }}" id="eq_{{ Str::slug($equip) }}"
                                {{ in_array($equip, old('equipamiento', $aula->equipamiento ?? [])) ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea name="descripcion" rows="3" placeholder="Descripción adicional...">{{ old('descripcion', $aula->descripcion) }}</textarea>
            </div>
