...
@section('title', 'Editar Rol y Permisos: ' . ucfirst($role->name))
...
<div class="space-y-6 max-w-5xl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-700 dark:text-slate-200 font-semibold">Editar Rol</span>
            </div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
                <span>🛡️</span> Configurar Permisos del Rol: <span class="text-sky-600 dark:text-sky-400">{{ ucfirst($role->name) }}</span>
            </h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                Selecciona los permisos que tendrán los usuarios asignados a este rol agrupados por módulo.
            </p>
        </div>
    ...
    @if($role->name === 'admin')
        <div class="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/80 text-amber-800 dark:text-amber-300 text-xs font-semibold flex items-center gap-2 shadow-xs">
            <span>⚠️</span>
            <span>El rol <strong>Admin</strong> es el superadministrador del sistema y mantiene acceso total a todos los módulos y funciones.</span>
        </div>
    @endif

    <form action="{{ route('roles.update', $role) }}" method="POST" id="editRoleForm" class="space-y-6">
        @csrf
        @method('PUT')
